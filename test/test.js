...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof dnanstdev, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( dnanstdev.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the standard deviation of a one-dimensional ndarray, ignoring NaN values', function test( t ) {
	var correction;
	var expected;
	var opts;
	var x;
	...
	x = new Float64Array( [ 1.0, -2.0, NaN, -4.0, 5.0, NaN, 0.0, 3.0 ] );
	correction = scalar2ndarray( 1.0, opts );

	v = dnanstdev( [ vector( x, x.length, 1, 0 ), correction ] );
	expected = sqrt( 53.5 / 5 );
...
tape( 'if provided an empty ndarray, the function returns `NaN`', function test( t ) {
	...
	v = dnanstdev( [ vector( x, 0, 1, 0 ), correction ] );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
...
tape( 'if provided a correction argument yielding `N-correction` less than or equal to `0`, the function returns `NaN`', function test( t ) {
	...
	v = dnanstdev( [ vector( x, 1, 1, 0 ), correction ] );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
...
tape( 'the function supports one-dimensional ndarrays having non-unit strides', function test( t ) {
	...
	v = dnanstdev( [ vector( x, 5, 2, 0 ), correction ] );
	expected = 2.5;
	t.strictEqual( v, expected, 'returns expected value' );
...
tape( 'the function supports one-dimensional ndarrays having negative strides', function test( t ) {
	...
	v = dnanstdev( [ vector( x, 5, -2, 8 ), correction ] );
	expected = 2.5;
	t.strictEqual( v, expected, 'returns expected value' );
...
tape( 'the function supports one-dimensional ndarrays having non-zero offsets', function test( t ) {
	...
	v = dnanstdev( [ vector( x, 5, 2, 1 ), correction ] );
	expected = 2.5;
	t.strictEqual( v, expected, 'returns expected value' );
